...
import { SendIntentParams } from "@/modules/order";
...
export class Validator {
  ...
  static validateIntentParams(params: SendIntentParams): void {
    this.validateNonce(params.nonce);
    this.validateSignature(params.signature);
  ...
  static validateMarketAddress(address: string): void {
    if (!address || typeof address !== "string") {
      throw new ValidationError(
        "Market address must be a non-empty string",
        "market_addr",
      );
    }
    if (!address.startsWith("0x")) {
      throw new ValidationError(
        "Market address must start with 0x",
        "market_addr",
      );
    }
  }
