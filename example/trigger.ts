...
import { depositToTrading, ensureRegistration, fundAccount, getAptosClient } from "./faucet";
...
const STRICT_TRIGGER_SOURCE = (env.STRICT_TRIGGER_SOURCE ?? "true").toLowerCase() !== "false";
const LOCAL_AUTO_BOOTSTRAP =
	NETWORK === "local" && (env.LOCAL_AUTO_BOOTSTRAP ?? "true").toLowerCase() !== "false";
...
function roundToStep(value: number, step: number, mode: "up" | "down" | "nearest"): number {
...
function approxEqString(a: string | null | undefined, b: string, eps = 1e-9): boolean {
	if (!a) return false;
...
async function eventually<T>(
	...
	if (lastError instanceof Error) {
		throw new Error(`${label} timed out (${timeoutMs}ms). Last error: ${lastError.message}`);
	}
	throw new Error(
...
async function authorizeClient(label: string, privateKeyRaw: string): Promise<AuthContext> {
...
async function authorizeWithRetry(privateKeyRaw: string, client: EkidenClient): Promise<string> {
	...
	if (lastError instanceof Error) {
		throw new Error(`Failed to authorize after bootstrap: ${lastError.message}`);
	}
...
async function getTradingAvailableBalance(
	...
	const balances = await client.account.getBalance();
	const row = balances.list.find((b) => b.sub_account_address === tradingSubAddress);
	if (!row) return 0;
	return parseNumber(row.available_balance, `available_balance:${tradingSubAddress}`);
}

async function bootstrapLocalTrader(label: string, privateKeyRaw: string): Promise<void> {
	const normalized = normalizePrivateKey(privateKeyRaw);
	...
	try {
		try {
			...
			const existingTrading = await getTradingAvailableBalance(client, trading.address);
		...
		await ensureRegistration(client, root, systemInfo, fundingAccount, tradingAccount, aptos);
		client.setToken(await authorizeWithRetry(normalized, client));

		const tradingBeforeDeposit = await getTradingAvailableBalance(client, trading.address);
		...
		try {
			...
			await fundAccount(client, root, systemInfo.quote_asset_metadata, aptos, 200 * 10 ** 6);
			await ensureAptBalanceForFees(client, root.accountAddress.toString());
			await depositToTrading(
				client,
				root,
...
async function loadMarketContext(
	...
	return {
		...
		tickSize: parseNumber(instrument.price_filter.tick_size, `${symbol}.tick_size`),
		tickDecimals: decimalPlaces(instrument.price_filter.tick_size),
		qtyStep: parseNumber(instrument.lot_size_filter.qty_step, `${symbol}.qty_step`),
		qtyDecimals: decimalPlaces(instrument.lot_size_filter.qty_step),
	};
}

function priceAt(ctx: MarketContext, base: number, mode: "up" | "down"): string {
	return fmt(roundToStep(Math.max(base, ctx.tickSize), ctx.tickSize, mode), ctx.tickDecimals);
}

function pickQty(ctx: MarketContext): string {
	const minQty = parseNumber(ctx.instrument.lot_size_filter.min_order_qty, "min_order_qty");
	const candidate = env.TRIGGER_QTY ? parseNumber(env.TRIGGER_QTY, "TRIGGER_QTY") : minQty * 2;
	return fmt(roundToStep(Math.max(minQty, candidate), ctx.qtyStep, "up"), ctx.qtyDecimals);
}
...
async function runStopMarketFilled(
	...
	const makerPrice = priceAt(
		ctx,
		isBuy ? Math.max(ctx.bestAsk, ctx.mark) * 1.002 : Math.min(ctx.bestBid, ctx.mark) * 0.998,
		isBuy ? "up" : "down"
	);
...
export async function runTriggerExample(): Promise<void> {
	const takerPk = env.PK ?? (NETWORK === "local" ? env.APTOS_E2E_PRIVATE_KEY : undefined);
	const makerPk =
		env.PK_MAKER ?? (NETWORK === "local" ? env.APTOS_E2E_SUB_PRIVATE_KEY : undefined);
	if (!takerPk) throw new Error("Missing PK (Trader A root private key).");
	if (!makerPk) throw new Error("Missing PK_MAKER (Trader X root private key).");
	if (takerPk === makerPk) throw new Error("PK and PK_MAKER must be different accounts.");
	...
	const instrumentBySymbol = new Map(instrumentsResp.list.map((item) => [item.symbol, item]));
	...
	if (!symbol) {
		const preferred = tickers.list
			...
			.filter((v): v is { symbol: string; spread: number; tick: number } => Boolean(v))
			.sort((a, b) => b.spread - a.spread);
		symbol = preferred.find((m) => m.spread >= m.tick)?.symbol ?? preferred[0]?.symbol;
	}
	if (!symbol) throw new Error("Could not pick a symbol. Set SYMBOL explicitly.");

	const instrument = instrumentBySymbol.get(symbol);
	if (!instrument) throw new Error(`Instrument info missing for symbol ${symbol}`);
	...
	const unsubscribeOrder = traderA.client.privateStream?.subscribeOrder((rows) => {
	...
	const unsubscribeExecution = traderA.client.privateStream?.subscribeExecution((rows) => {
		if (!Array.isArray(rows)) return;
		for (const raw of rows as Array<Record<string, unknown>>) {
			...
			wsExecutions.push({
				...
				exec_price: typeof raw.exec_price === "string" ? raw.exec_price : undefined,
			});
			if (LOG_TRIGGER_WS) {
				console.log(
	...
	const unsubscribePosition = traderA.client.privateStream?.subscribePosition((rows) => {
		if (!Array.isArray(rows)) return;
	...
	try {
		...
		const filled = await orderByLink(traderA, market.symbol, stopBuyLink, "StopOrder");
		assert(filled.stop_order_type === "Stop", "Expected stop_order_type=Stop");
		const row = await assertSingleRowContinuity(traderA, market.symbol, stopBuyLink);
		assert(
			row.order_status === "Filled",
			"Expected single-row history continuity ending in Filled"
		);
		await sleep(500);
		const rowAfter = await assertSingleRowContinuity(traderA, market.symbol, stopBuyLink);
		assert(
		...
		const byId = await placeStopOrder(traderA, {
			...
			triggerBy: "MarkPrice",
			triggerDirection: "Up",
			triggerPrice: priceAt(market, Math.max(market.mark, market.last) * 1.25, "up"),
		});
		await waitOrderStatus(traderA, market.symbol, cancelByIdLink, ["Untriggered"]);
		...
		await waitOrderStatus(traderA, market.symbol, cancelByLinkLink, ["Untriggered"]);
		await traderA.client.trade.cancelOrder({
			sub_account_address: traderA.subAccount,
			symbol: market.symbol,
			order_link_id: cancelByLinkLink,
		});
		await waitOrderStatus(traderA, market.symbol, cancelByLinkLink, ["Canceled"]);
		...
		await placeStopOrder(traderA, {
			...
			triggerBy: "MarkPrice",
			triggerDirection: "Up",
			triggerPrice: priceAt(market, Math.max(market.mark, market.last) * 1.2, "up"),
		});
		...
		assert(links.has(stopLink), "StopOrder filter must include stop conditional order");
		assert(!links.has(activeLink), "StopOrder filter must exclude regular active order");
		...
		if (markIndexGap < market.tickSize) {
			...
			const sentinelDirection: TriggerDirection = markAboveIndex ? "Up" : "Down";
			const sentinelSide: Side = markAboveIndex ? "Buy" : "Sell";
			const sentinelTrigger = priceAt(
				market,
				markAboveIndex ? market.index + market.tickSize : market.index - market.tickSize,
		...
		await eventually(
			...
			async () => {
				...
				return (
					resp.list.find(
						(p) =>
							p.symbol === market.symbol && parseNumber(p.size, "position.size") > 0
					) ?? null
				);
			},
		...
		await eventually(
			...
			async () => {
				const wsEvent =
					[...wsPositionEvents].reverse().find((p) => p.symbol === market.symbol) ?? null;
				const restPosition = await traderA.client.position.getPositionInfo({
					sub_account_address: traderA.subAccount,
					symbol: market.symbol,
					limit: 20,
				});
				const restEvent = restPosition.list.find((p) => p.symbol === market.symbol) ?? null;
		...
		assert(freshUntriggered.length >= 2, "Expected at least two new TP/SL untriggered orders");

		const wsStatuses = [...new Set(wsTriggerOrders.map((o) => o.order_status))].sort();
		console.log(
